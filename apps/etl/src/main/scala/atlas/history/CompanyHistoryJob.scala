...
object CompanyHistoryJob {
  ...
  def refresh(spark: SparkSession, config: AtlasConfig, bundleId: String): CompanyHistoryResult = {
    ...
    try {
      ...
      val result = if (parquetExists(spark, currentPath)) compare(spark, config, spark.read.parquet(currentPath.toString), candidate, bundleId)
      else seed(spark, config, candidate, bundleId)
      candidate.write.mode("overwrite").parquet(currentPath.toString)
      result
    } finally candidate.unpersist()
  }

  private def seed(spark: SparkSession, config: AtlasConfig, candidate: DataFrame, bundleId: String): CompanyHistoryResult = {
    val count = candidate.count()
    writeSummary(spark, config, bundleId, None, None, count, 0L, 0L, 0L, 0L)
  ...
  private def compare(
    ...
    try {
      ...
      writeSummary(spark, config, bundleId, fromRelease, Some(previous), current, inserted, updated, removed, total)
  ...
  private def writeSummary(
    ...
    Seq((
      ...
      "malformed_count", "duplicate_count", "reference_miss_count", "bundle_id", "outcome", "calculated_at"
