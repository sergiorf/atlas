...
object TrashPurgeService {
  private val TimestampPattern = "^([0-9]{4}-[0-9]{2}-[0-9]{2})T([0-9]{2})([0-9]{2})([0-9]{2})([0-9]*)Z$".r
  private val KnownOperations = Set("release-drop", "stale-derived", "failed-rebuild", "full-rebuild-backup")

  def inspect(config: AtlasConfig, olderThanDays: Int = 7, now: Instant = Instant.now()): TrashPurgeResult = {
  ...
  def force(config: AtlasConfig, olderThanDays: Int = 7, now: Instant = Instant.now()): TrashPurgeResult =
  ...
  private def inspectGeneration(
    ...
    if (age.exists(_.compareTo(Duration.ofDays(olderThanDays.toLong)) < 0)) reasons += s"younger than $olderThanDays days"
    ...
    if (operationType == "full-rebuild-backup") {
      manifestResult.foreach(manifest => validateFullRebuild(config, manifest).foreach(reasons += _))
    }
    ...
    TrashGeneration(operationDir, timestamp, operationType, age, bytes, reasons.isEmpty, reasons.distinct.toSeq)
  }
  ...
  private def operationDirectories(trashRoot: Path): Seq[(Path, Path)] = {
    ...
    list(trashRoot).flatMap { timestampDir =>
      if (!Files.isDirectory(timestampDir, LinkOption.NOFOLLOW_LINKS)) Seq(timestampDir -> timestampDir)
      else list(timestampDir).map(timestampDir -> _)
    }.toVector.sortBy(_._2.toString)
  }
  ...
  private def readOrInfer(root: Path, timestamp: Option[Instant]): Either[String, TrashManifest] = {
    ...
    if (Files.exists(manifest, LinkOption.NOFOLLOW_LINKS)) {
      ...
      catch { case NonFatal(error) => Left(s"malformed trash manifest: ${Option(error.getMessage).getOrElse(error.getClass.getName)}") }
    } else {
      ...
      operation.map(value => TrashManifest(value, timestamp.getOrElse(Instant.EPOCH), Seq.empty, Seq.empty, list(root).map(_.getFileName.toString)))
  ...
  private def validateTree(trashRoot: Path, root: Path): Seq[String] = {
    ...
    if (Files.exists(root, LinkOption.NOFOLLOW_LINKS)) {
      ...
      try stream.iterator().asScala.foreach { path =>
        ...
        if (!path.toAbsolutePath.normalize().startsWith(normalizedTrash)) reasons += s"path escapes configured trash root: $path"
  ...
  private def validateFullRebuild(config: AtlasConfig, manifest: TrashManifest): Seq[String] = {
    ...
    if (replacements.isEmpty) reasons += "legacy full-rebuild backup has no replacement expectations"
    replacements.filterNot(path => Files.exists(path, LinkOption.NOFOLLOW_LINKS)).foreach(path => reasons += s"required active output is missing: $path")
    ...
    val forbidden = statuses.flatMap(status => status.inputPaths ++ status.outputPath.toSeq ++ status.qualityWarnings.map(_.reportPath))
    ...
    val successfulLayers = statuses.filter(status => status.status == "success" || status.status == "success_with_warnings").map(_.layer).toSet
  ...
  private def parseTimestamp(value: String): Option[Instant] = value match {
    case TimestampPattern(date, hour, minute, second, fraction) =>
      ...
      try Some(Instant.parse(s"${date}T$hour:$minute:$second${decimal}Z")) catch { case NonFatal(_) => None }
    case _ => None
  }

  private def treeBytes(root: Path): Option[Long] = try {
    val stream = Files.walk(root)
    try Some(stream.iterator().asScala.filter(path => Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)).map(Files.size(_)).sum)
    finally stream.close()
  } catch { case NonFatal(_) => None }

  private def deleteTree(root: Path): Unit = {
    if (Files.isSymbolicLink(root)) throw new IllegalStateException(s"Refusing to delete symbolic link $root")
    val stream = Files.walk(root)
    try stream.iterator().asScala.toVector.sortBy(_.getNameCount).reverse.foreach(Files.delete(_))
    finally stream.close()
  }

  private def pruneEmptyTimestampDirectories(trashRoot: Path): Unit = list(trashRoot).foreach { path =>
    if (Files.isDirectory(path, LinkOption.NOFOLLOW_LINKS) && list(path).isEmpty) Files.delete(path)
  }
  ...
  private def renderTable(headers: Seq[String], rows: Seq[Seq[String]]): String = {
    ...
    (headers +: rows).map(row => row.indices.map(index => row(index).padTo(widths(index), ' ').mkString).mkString("  ").replaceAll("\\s+$", "")).mkString("\n")
  }
}
