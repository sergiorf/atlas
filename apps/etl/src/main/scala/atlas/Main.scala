...
object Main {
  private final case class Cli(command: String, configPath: String = "conf/application.conf")
  ...
  private def parseArgs(args: List[String]): Cli = args match {
    ...
    case _ => throw new IllegalArgumentException("Usage: atlas.Main ingest-receita-estabelecimentos [--config PATH]")
  }
}
