...
- **Status:** Implemented
- **Owner:** Receita reference pipeline
- **Contract level:** Internal silver reference contract

These release-scoped tables are implemented within atomic bundle generations.
...
Each monthly bundle produces one release-scoped dimension for `cnae`, `municipality`,
...
The bundle-relative path is `data/silver/receita/references/<dimension>/release=YYYY-MM`. Code is the
