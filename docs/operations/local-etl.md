...
Bronze ingestion records its latest attempt in the local [status registry](../manual/status_registry.md). A failed status is diagnostic only: investigate and rerun the ingest, which replaces the record for the same source, dataset, snapshot, and layer. Do not edit status files to claim a successful run.
