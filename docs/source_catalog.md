...
| Receita Federal / CNPJ references | `CNAE`, `Municipios`, `Naturezas Juridicas`, `Paises`, `Qualificacoes de Socios`, and `Motivos de Situacao Cadastral` | Decode official activity, municipality, legal-nature, country, responsible-party, and registration-reason codes | **Active planned tranche** / v0.3a | Public bulk download; no access fee | Published with CNPJ snapshots | **Review required** | [CNPJ catalog and official two-field layouts](https://www.gov.br/receitafederal/dados/cnpj-metadados.pdf/@@download/file) | [Source interpretation](specs/datasets/receita-company-reference-sources.md) and [reference dimensions](specs/schemas/receita-reference-dimensions.md) |
...
Only Receita CNPJ `Estabelecimentos` is supported as a transformed dataset. The active company-data
rows have implemented raw acquisition and source-manifest verification but remain unsupported for
bronze and later behavior. The establishment input, acquisition, refresh,
